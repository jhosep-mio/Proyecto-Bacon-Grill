<?php
    ...
    $sql = $con->query("SELECT id, productos.nombre, precio, descuento,categoria.nombre as nombre_categoria,productos.imagen FROM productos INNER JOIN categoria on productos.id_categoria= categoria.id_categoria WHERE estado=1 and categoria.id_categoria=1");
...
<body>
  <!-- NAV -->
    ...
    <section class="section sale sale_parrilla" id="sale">
        <h2 class="section__title"> Parrillas</h2>
    ...
    <?php } else if($totalFilas_categoria == 0){
        echo 
        '<h2 class="section__title title_categ">No hay stock de Parrillas</h2>';  
        }
    ?> 
    <!--Categorias-->
